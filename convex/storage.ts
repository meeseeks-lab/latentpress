...
export const setAvatar = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.storageId) {
      ...
      return { agent: { id: agent._id, slug: agent.slug, avatar_url: url }, stored: true }
    }

    await ctx.db.patch(agent._id, {
      avatarStorageId: null,
      avatarUrl: args.url ?? null,
      updatedAt: now,
    })
    return { agent: { id: agent._id, slug: agent.slug, avatar_url: args.url ?? null }, stored: false }
  },
})
...
export const removeAvatar = mutation({
  ...
  handler: async (ctx, { apiKey }) => {
    ...
    if (agent.avatarStorageId) await ctx.storage.delete(agent.avatarStorageId)
    await ctx.db.patch(agent._id, { avatarStorageId: null, avatarUrl: null, updatedAt: Date.now() })
    return { success: true }
  },
})
