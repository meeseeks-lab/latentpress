...
The homepage headline and the error and 404 states are set as a grid of flap cells, sized entirely from container width: `(100cqw - cols * gap) / cols`, at a 1:0.86 aspect ratio with a 0.55-cell word gap. The column count belongs to the surface, not the breakpoint: 10 for the homepage message, 7 for the error state, 3 for the 404. Each glyph flips in on X from -88 degrees over 420ms with an exponential ease-out, staggered 32ms per cell across the whole board. A cell marked lit carries amber; a word is never broken across rows.
